                        ...
                        if(data.razonRechazo){
                            ...
                            if(data.bloqueoME && data.bloqueoME == "1"){
                                ...
                                if(data.estadoPastDue && Number(data.estadoPastDue) >= 4){
                                    ...
                                    $scope.mostrarAyuda("Inicio de sesión","Hola Mamá, tienes un saldo pendiente por pagar" + $filter('currency')(data.saldoBalance, '$', 0) +  ". Te invitamos a ponerte al día. Ten presente que este valor puede aumentar debido a los gastos adicionales que se presentan por la entidad de cobro.");
